...
int getmenu(const char *a,int o,int EXIT){
    ...
    char input[MAXOPTION];
     ...
     matrix[id1][j] = '\0';
      //
    do {i=0;if(o<=5||  ((count2da(matrix,o)+7)*2+2+TABS+TAB*TABS) > 80)
        {     if(count2da(matrix,o)%2!=0) {printf("\n");repchar(TAB,'\t');printf(JOINTL);repchar(((count2da(matrix,o)+4)/2),ROW);printf(" Menu ");repchar(((count2da(matrix,o)+4)/2-1),ROW);printf(JOINTR"\n");}
                ...
                for(id2 = 0 ; id2 < o;id2++){repchar(TAB,'\t');printf(COLL"%3d. %-*s"COLR"\n",id2+1,count2da(matrix,o)+1,matrix[id2]);}

                if(EXIT == 1){repchar(TAB,'\t');printf(COLL"  0. %-*s"COLR"\n",(count2da(matrix,o)+1),"Exit");}
        ...
        else
        {
            printf("\n");repchar(TAB,'\t');printf(JOINTL);repchar(count2da(matrix,o)+4,ROW);printf(" Menu ");repchar(count2da(matrix,o)+4,ROW);printf(JOINTR"\n");
                for(id2 = 0 ; id2 < o;id2++){if(flag == 0){repchar(TAB,'\t');printf(COLL"%3d. %-*s",id2+1,count2da(matrix,o)+1,matrix[id2]);flag++;}
                 else {printf("%3d. %-*s"COLR"\n",id2+1,count2da(matrix,o)+1,matrix[id2]);flag=0;}}
                if(EXIT == 1) if(o%2 == 0){repchar(TAB,'\t');printf(COLL"  0. %-*s"COLR"\n",(count2da(matrix,o)+1)*2+5,"Exit");} 
        ...
        //
        if(option <= o && option >= 0 && checkan(input,i)) {
	  if(option != 0) {
	       repchar(TAB,'\t');
